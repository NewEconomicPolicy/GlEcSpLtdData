...
def commonSection(form, grid, irow):
    ...
    w_use_pi_nc.clicked.connect(form.adustPiCsv)
    ...
    # Joe and Marta addition for PI CSV
    # =================================
    irow += 1
    w_use_pi_csv = QCheckBox('Use pi CSV file')
    w_use_pi_csv.setToolTip(helpText)
    w_use_pi_csv.clicked.connect(form.adustPiNc)
    grid.addWidget(w_use_pi_csv, irow, 0)
    form.w_use_pi_csv = w_use_pi_csv

    w_pi_csv = QPushButton("Plant input CSV")
    helpText = 'Select CSV file of plant inputs'
    w_pi_csv.setToolTip(helpText)
    w_pi_csv.setEnabled(True)
    grid.addWidget(w_pi_csv, irow, 1)
    w_pi_csv.clicked.connect(form.fetchPiCsvFile)

    lbl_pi_csv = QLabel()
    grid.addWidget(lbl_pi_csv, irow, 2, 1, 4)
    form.w_lbl_pi_csv = lbl_pi_csv
